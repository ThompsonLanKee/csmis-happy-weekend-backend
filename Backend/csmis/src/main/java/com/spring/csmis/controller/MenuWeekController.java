...
@RestController
@RequestMapping("/admin/menuweek")
public class MenuWeekController {

    @Autowired
    private MenuWeekService menuWeekService;

    // Add new menu week
    @PostMapping("/add")
    public ResponseEntity<MenuWeekDTO> createMenuWeek(@RequestBody MenuWeekDTO menuWeekDTO) {
        MenuWeekDTO createdMenuWeek = menuWeekService.addMenuWeek(menuWeekDTO);
        return ResponseEntity.ok(createdMenuWeek);
    }
    ...
    @GetMapping("/prices")
    public List<MenuWeekEntity> getMenuPricesForMonth(@RequestParam int month, @RequestParam int year) {
        return menuWeekService.getMenuPricesForMonth(month, year);
    }
}
